...
/**
 * Angular equivalent of React's useOnUploadStart hook.
 * Sets up a listener for file upload start events.
 */
@Injectable({
  providedIn: 'root'
})
export class UseOnUploadStartService {
  private destroyRef = inject(DestroyRef);

  /**
   * Sets up an onUploadStart listener on the editor.
   * The listener will be automatically cleaned up when the component/service is destroyed.
   *
   * @param callback - Function to call when file upload starts
   * @param editor - Optional editor instance (uses NgxBlocknoteService if not provided)
   */
  setupOnUploadStartListener(
    ...
    // Set up the onUploadStart listener
  ...
  /**
   * Creates an effect that automatically sets up and tears down the onUploadStart listener
